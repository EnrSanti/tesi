...
Another distinction can be made on whether the relation is dependent not only on the functions but also on the variables of the functions or not. If the case is the latter then the differential equation is said to be autonomous.\newline
...
An ordinary differential equation, or ODE, is a kind of differential equation where the \emph{unknown} function only depends on one independent variable (usually referred to as time). The domain of the \emph{unknown} function is then an interval $I \subset R$. The most general form an ODE can be written as: $F(t,y(t), y’(t) ... y^{(k)}(t))=0$ \cite{odeIntro} where $t$ is the independent variable, y the \emph{unknown} function, $ y^{(i)}(t) $ the i-th derivative of $y$ and $F$ a function from $A \subseteq R^{(k+2)} \rightarrow R$. \newline
...
As in many other mathematical fields a geometrical interpretation associated with the concepts helps to obtain a better understanding of the topic.
One of the first geometric concepts linked to differential equations is the Trajectory diagram.    This diagram, in relation to a given ODE, presents on one of its axis the independent variable, and on the other the \emph{unknown} function. What this diagram shows is a vector field\footnote{i.e. an assignment of a vector to each point in a given space}, where in each point, the slope of the corresponding vector represent the value the first derivative of the \emph{unknown} function.\newline
...
For what concerns systems of autonomous ordinary differential equations usually the phase or orbit plane is presented, this plane doesn't present the independent variable axis (e.g. the time axis), allowing to study the dynamics of a system independently from the time perspective (i.e. study the orbits).
...
An equilibrium, so a solution constant on the domain, for the equation (regardless the value of the parameter r) is obtained with the value $y_0=0$. Plotting the solution of the relative IVP (with different values of r) will always result in a straight line on the trajectory plane.
...
A dynamical system is a system whose state varies as the time varies \cite{arrowsmith1990introduction} inside a phase space. A phase space can be informally described as all the values the dynamical system considered can assume as the time progresses. These systems are used to model the evolution of a phenomenon.\newline
A fist major distinction of these systems is related to how the time is considered, if it is considered as a series of discrete intervals (i.e. $ t \in N $) then these are discrete dynamical systems, on the other hand if the time is considered continuous (i.e. $ t \in R $) then these are continuous dynamical systems.
...
A continuous dynamical system can be defined by a set of times $T \subset R$, a state space $X$ and a family of maps $T(t): X \rightarrow X$ such that $T(0) = Ix$ (where Ix is the identity map) and for all $ times t1,t2 \in T$   $ T(t1+t2)=T(t1) \circ T(t2)$ \textcolor{red}{[citazione }.
...
The idea of stability of a solution is both informally and rigorously described in \cite{chicone}, the latter can be defined as: a solution for a differential equation is stable whether other solutions close to the solution considered remain relatively close.
\newline\newline
