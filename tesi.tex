...
Then, $F_M$, defined as $F(R_M(y_M))$ with $y_M \in Y_M$ approximates $F$ \ref{ddesystem}, the corresponding discretization of $A_F$ of index $M$ is the operator $A_{F,M}$ defined as $A_{F,M}(y_M)=(G_M(y_M),\Upsilon_M)$\textcolor{orange}{definizione AFM} and the abstract differential equation can be seen as an ODE on $Y_M$. The abstract IVP \eqref{adesys} can be turned into:
\vspace{0.5em}
\begin{equation}
\begin{cases}
...
\end{equation}
\vspace{0.5em}
The initial IVP is approximated by an ODE IVP of dimension $d*(M+1)$ equations. 
