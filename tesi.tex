...
Delay differential equations can be seen as a generalization of ODEs, and in the mathematical modelling context are crucial to describe certain phenomena not representable by ODEs. Informally a delay differential equation can be seen as an ODE in which the function $F$ takes into account not only the value of the unknown function it's derivatives at the current time $t$ considered but also at previous times (e.g. $t-\tau$ where $\tau \in {\rm I\!R}$).
...
In order to find a specific solution to a first order delay equation differently from what happened with an ODE, it's not sufficient to specify a single initial value, but an entire function on the interval $[-\tau_{max},0]$ where $\tau_{max}$ is the maximum value among the delays values in the equation.
