...
Chapter \ref{cap6} we describe the fundamental aspect of the current version of MatCont's GUI and how it works while in in Chapter \ref{cap7} we will focus on the new capabilities of the extended version of the extended version of MatCont, what are the new capabilities of this software and how the development has been carried out. Finally in Chapter \ref{conclusions} we conclude with some remarks on the future additional tools to implement to analyse the dynamics of renewal equations too.
\newline
\newline
...
Dynamical systems describe the evolution in time of phenomena and find application in several fields, from engineering to biology (\cite{arrowsmith1990introduction,kuznetsov}).
...
In this thesis we will treat only continuous dynamical systems defined by ordinary differential equation (ODE) and delay equations, so from here we refer to them simply as dynamical systems.
...
We consider autonomous ordinary differential equations of the form:
\begin{equation}
\label{odeeq}
y'(t)=F(y(t)), \;\;\; t \geq 0
\end{equation}
where $t$ is the independent variable, $y$ the unknown function (so $y(t) \in R^d$  is the state) and $ {\rm I\!R}^{d}$ the state space with $d \geq 1$,  and $F : A \subseteq  {\rm I\!R}^{d} \rightarrow  {\rm I\!R}$.
Solving a differential equation (when it is possible) means finding a set of functions satisfying the relationship on an interval. To obtain a unique solution additional conditions are needed. Here we consider the following initial value problem (IVP):
\begin{equation} 
\begin{cases}
\label{IVPode}
y'(t)=F(y(t)),\;\;\; t \geq 0\\
...
\end{cases}\,.
\end{equation}
Under suitable assumptions on the right hand side $F$, the IVP \eqref{IVPode} has a unique solution $y(t)=\phi^t(y_0)$, for $t \in [0,t_f]$. Moreover it can be proven that the solution continuously depends on the initial value $y_0$ \cite{chicone}. Note that the state space is ${\rm I\!R}^d$ so the equation \eqref{odeeq} defines a finite-dimensional dynamical system.
\newline
A trajectory of a solution of an ODE, in particular for a dynamical system, consists of the set of the values of the unknown function in the interval and the values of the independent variable itself (i.e. ${(t,y(t)): t \in I}$).
Related to this concept, one of the first geometric visualizations for ODEs is the trajectory diagram. This diagram, in relation to a given ODE, presents on one of its axes the independent variable, and on the other the unknown function, for what concerns systems of ODEs components (unknown functions) is considered separately in different diagrams. What this diagram shows is a vector field (i.e. an assignment of a vector to each point in a given space), where at each point, the slope of the corresponding vector represents the value of the first derivative of the unknown function at that specific point.\newline
...
The general solution of the equation is $\phi^t(y_0)=y_0 e^{rt-t_0}$ for $t \geq 0$ where $y_0$ is the initial value chosen for $y(t_0)$.\newline
...
\begin{equation}  
\label{logisticEQsol}
\phi^t(y_0)=\frac{K}{1+(\frac{K}{y_0}-1)e^{-r(t-t_0)}}
\end{equation}
Where $y_0$ is the value of $y(t_0)$ in the related IVP.\newline
...
\newline 
The geometric interpretation allows to qualitatively study IVPs without analytically solving them, giving insights on the problem \cite{chicone}.
...
Delay differential equations can be seen as a generalization of ODEs and in the mathematical modelling context are crucial to describe certain phenomena not representable by ODEs. With reference to \cite{diekmann2012delay} a delay equation is a rule for extending a function of time towards the future
on the basis of the (assumed to be) known history.
Let $y_t(\theta):=y(t+\theta)$ with $\theta \in [-\tau,0]$ be the history of the unknown function, which also represents the state, a delay equation can then be defined as $y'(t)=F(y_t), t \geq 0$ where the state space is $X=C([-\tau, 0]; {\rm I\!R}^d)$. Note that state $y_t$ is an infinite-dimensional object (a function) \cite{diekmann2012delay}.
\newline 
\newline 
...
In order to find a specific solution to a first order delay equation differently from what happens with an ODE, it's not sufficient to specify a single initial value, but an entire function on the interval $[-\tau,0]$ where $\tau$ is the maximum delay in the equation is required. An IVP will then result in a system resembling:
\begin{equation} 
\begin{cases}
\label{ddesystem}
y'(t)=F(y_t),   \;\;\; t\geq0\\
...
\label{stability}
The idea of stability of a solution (given an initial point) is both rigorously and intuitively described in \cite{chicone}, it can be intuitively defined as: the property that makes solutions near to the considered solution remain relatively close as the system evolves. On the other hand, if a solution isn't stable it is said to be unstable.
...
\newline\newline
When taking into consideration systems depending on one or more parameters such as \eqref{malthuseq} it can be studied how changes in the value of a single parameter affect the solution of the system itself.
...
\vspace{0.8em}
$p \in {\rm I\!R}^m$ represents the vector of the $m$ parameters presented by $y(t)$, only real value parameters are being considered in this thesis.\newline 
The graph showing how changes in the parameter change (qualitatively) the behavior of the solutions is the \emph{bifurcation diagram} \cite{chicone} and it can be represented as a Euclidean plane presenting on an axis the parameter chosen for the bifurcation analysis and on the other axis the coordinates for the orbit diagram of the system.
\newline
A bifurcation could be described as the values of a considered parameter at which changes in the value of the parameter correspond to qualitative changes in the behavior of the solution. Depending on which aspect of the system's solution changes, bifurcation points can be classified into different classes as described in \cite{kuznetsov}. An example may be the Hopf bifurcation: A value of the considered parameter at which one equilibrium changes stability and a limit cycle\footnote{An isolated and closed curve which other orbits approach to or are repelled by it as time progresses} arises.


\section{From DDEs back to ODEs }

The first step to discretize a delay equation to a system of ODEs is to rewrite a delay differential equation as an abstract differential equation. The generic IVP \ref{ddesystem} is equivalent to the following abstract IVP \cite{DDEdisc}:
\begin{equation} 
\begin{cases}
\label{adesys}
v'(t)=A_F(v(t)),  \;\;\; t\geq0\\
...
\end{cases}\,.
\end{equation}
where $A_F$ is the generator associated to the semigroup generated by the solution operators $\phi$ parametrized in $\psi$.\newline
...
Consider now $M \in {\rm I\!N}\backslash\{0\}$ distinct points in $[-\theta,0)$ and a last point equal to 0, for example the $M+1$ Chebyshev extrema\footnote{real values in [-1,1] which can be calculated as $x_k=cos(\frac{k}{n} \pi), k \in [1,n]$ where $x_k$ is the $k$-th extrema and $n$ is the number of nodes needed.} in $[-\theta,0]$ and let $d$ be the dimension of the original system. The idea is to approximate the state to a finite dimensional object by representing it through the polynomial interpolation on the nodes. The space $X_M=(R^{d(M+1)})$ is the discretization of the state space $X$ of the original system of index $M$, every state $x \in X$ is now represented by a vector $x_M \in X_M$ with the components of the vector denoting the state of each of the $d$ equations at the instant specified by the corresponding node.
The reconstruction operator $R_M$ associates to a given vector in $X_M$ the $M$-degree polynomial in $X$ interpolating the values of the vector in the respective considered points. 
The approximated state will then be described by:
\begin{center}
\vspace{-1.2em}
\[R_M(x_M)(\theta)=\sum_{i=0}^{M} \ell_{M,i}(\theta)\;x_{M,i},  \;\;\;\;\; \theta  \in [-\tau, 0]\]
\vspace{0.0001em}
\end{center}
Where $x_{M,i}$ is the $i$-th component of $x_M$ and $\ell_{i,M}$ \cite{DDEdisc} are the Lagrange polynomials considered on the M+1 points chosen, i.e.:
\[\ell_{M,i}(\theta) = \prod_{k=0, k \neq i}^{M} \frac{\theta-\theta_{M,k}}{\theta_{M,i}-\theta_{M,k}},\;\;\;\;\; i=0,1,...,M\] 
\newline
Then $F_M$, defined as $F(R_M(x_M))$ with $x_M \in X_M$, approximates $F$ \ref{ddesystem}. The corresponding discretization of $A_F$ of index $M$ is the operator $A_{F,M}$ defined as $A_{F,M}(y_M)=(F_M(x_M),\Upsilon_M)$ where $\Upsilon_{M,i}= \frac{d}{d\theta_i}R_M(x_M)(\theta_i) $ for $i=1...M$. The abstract differential equation can be seen as an ODE on $X_M$. The abstract IVP \eqref{adesys} can be turned into:
\vspace{0.5em}
\begin{equation}
\begin{cases}
...
\end{equation}
\vspace{0.5em}
The initial IVP is approximated by an ODE IVP of dimension $d\cdot(M+1)$ equations. 
...
\item While a few software to carry out bifurcation analysis of delayed equations exist (for example DDEBiftool which can treat DDEs with discrete and state dependent delays), softwares for the study of ODE systems are generally more complete and the methods utilized more known.
\end{itemize}
The presented technique allows also to deal with distributed delays making possible for the user to easily study different kinds of systems.

 

\chapter{A brief introduction to MatCont}
\label{cap6}
MatCont is a software, based upon MATLAB, that provides functions and a graphic user interface to study the behavior of continuous dynamical systems defined by ODEs, particularly for what concerns the bifurcation analysis\cite{documentazione}.
\newline
...
During the development process some choices were made to allow the extended system to further be expanded easily in order to accept other families of delay equations, such as state dependent DDEs and renewal equations (REs).\newline
...
One of the routines mentioned is responsible for writing in the file some functions each returning a pre-computed matrix. For example one function returns the M+1\footnote{where M is the degree of the collocation polynomial} Chebyshev nodes in the interval $[-1,0]$.
\newline \newline
