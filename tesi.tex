...
There will be emphasys in describing the software extended, what are its new capabilities and how this task has been achieved.
...
A simple example of an ODE is represented by the logistic equation:  \begin{center}$y'(t)=r*y(t)*(1-y(t)) $ \end{center} \label{logisitcEQ} This equation\footnote{Where $r$ is a parameter.} is a non linear autonomous first order ODE which can be solved analyitically by separation of variables.
...
For what concenrs systems of autonomous ordinary differential equations usually the phase plane is presented, this plane doesn't present the independent variable axis (e.g. the time axis), allowing to study the dynamics of a system independently from the time prespective (i.e. study the orbits).
...
This binary file contains the struct that will be loaded in order to initialise the fields of the current struct representing the system when "Edit", or "Load" is pressed. The .mat file, on the other hand, contains functions such as \emph{fun-eval} and \emph{init} that will be called by MatCont when performing different computations on the system.
...
\newline  
On the other hand the lack of code documentation, such as comments or method contracts, especially in some regions of the code can make reading, understanding and subsequently modifying the software quite a difficult task.
...
\newline
Creating a method that was able to recognise the syntax was a process carried out during the first phase of the development and extended later on. It largely involved the usage of regular grammars.
\newline
\newline
...
All the parameters will still be typed in the parameters entry whether they are used to calculate a delay or not. This latter functionality has been modified in later development stages since originally the parameters showing up in the delays (e.g. like $\tau$\ref{tau}) had to be specified in the subwindow, alongside the number of equations discretizing the system. \newline
\newline
