...
Delay differential equations can be seen as a generalization of ODEs and in the mathematical modelling context are crucial to describe certain phenomena not representable by ODEs. Informally a delay differential equation can be seen as an ODE in which the function $F$ \ref{F} takes into account not only the value of the unknown function derivatives at the current time $t$ considered but also at previous times (e.g. $t-\tau$ where $\tau \in {\rm I\!R}$).
...
In order to find a specific solution to a first order delay equation differently from what happens with an ODE, it's not sufficient to specify a single initial value, but an entire function on the interval $[-\tau_{max},0]$ where $\tau_{max}$ is the maximum value among the delays values in the equation.
A first useful distinction, that will be helpful in chapter 7, is distinguishing between discrete and distributed delays. The first kind of delay takes the form of the unknown function at time $t$ minus (plus) some positive (negative) delay term, while a distributed delay presents the unknown function inside an integral with the delay term being the integration variable.\newline
...
The first concept is to rewrite a delay differential equation as an abstract differential equation. Without going into the details \cite{DDEdisc} the following system:
\begin{equation} 
\begin{cases}
\label{ddesystem}
y'(t)=F(y(t+\theta)),   \;\;\; t\geq\theta\\
y(t)=\psi(t), \;\;0 \leq t<\theta 
\end{cases}\,.
\end{equation}
...
\end{cases}\,.
\end{equation}
Where $A_F$ is the generator associated to the semigroup generated by the solution operators T parametrized in $\psi$.\newline
Consider now $M in N$ distinct points in $[-\theta,0)$ and a last point equal to 0, for example the $M+1$ extrema Chebyshev nodes in $[-\theta,0]$. Let $d$ be the dimension of the original system, the space $Y_M=(R^d)^(M+1)$ is the discretization of the state space Y of the original system of index M, every state $y \in Y$ is now represented by a vector in $Y_M$ with the components of the vector denoting the state of each of the $d$ equations at the instant specified by the corresponding node.
The reconstruction operator $R_M$ associates to a given vector in $Y_M$ the M-degree polynomial in $Y$ interpolating the values of the vector in the respective points considered. 
The discretization of $A_F$ of index $M$ is then the operator $A_{F,M}$ and the abstract differential equation can be seen as an ODE on $Y_M$. The initial IVP has now become an ODE IVP on $(M+1*d)$ equations.
