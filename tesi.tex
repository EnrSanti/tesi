...
A first distinction among differential equations is done on the number of indepentent variables the \emph{unknown} fucntion has, if these variables are more than one then partial differential eqautions are being considered. On the other hand if the independent variable is only one, the differential eqaution is called an ordinary differential equation. \newline
...
The maximum degree of the derivative of a differential equation is referred as the degree of the equation.
...
\section{Intial value problem}

\section{Some examples}

A simple example of an ODE is represented by the logistic equation:  \begin{center}$y'(t)=r*y(t)*(1-y(t)) $ \end{center}  This equation\footnote{Where $r$ is a parameter.} is a non linear autonomous first order ODE which can be solved analyitically by separation of variables.
...
\caption{The phase plane of $y'(x)=y(x)+1$, with one solution plotted}
\label{fig:autonomous}
\end{figure}
\begin{figure}[htp]
\centering
\includegraphics[scale=0.300]{imgTesi/vecFieldNonAut.png}
\caption{The phase plane of $y'(x)=y(x)+x$, with one solution plotted}
\label{fig:mainMatCont2}
\end{figure}

The two phase planes and the solution of the respective initial value problem (with $y(0)=1$) epmphasize the geometrical difference between the phase plane of an autonomous ODE Figure \ref{fig:autonomous}, where the vector associted to a given value of x remains the same for every value of y and the phase plane of a non-autonomous ODE, that doesn't present this property.
...
A continous dynamical system can be defined by a set of times $T \subset R$, a state space $X$ and a family of maps $T(t): X \rightarrow X$ such that $T(0) = Ix$ (where Ix is the identity map) and for all $ times t1,t2 \in T$   $ T(t1+t2)=T(t1) \circ T(t2)$ [citazione note Liessi].
...............
...............
...............
...............
...
MatCont is a software that provides functions and a graphic user interface to study the behaviour of dynamical systems, particuralry what concerns the bifurcations\cite{documentazione}.
\newline
The GUI and functions provided are independent, since the functionalities of MatCont can be used from the MatLab command line. 
\newline
In the following paragraphs the main aspects of MatCont's GUI and how the user could interact with it will be discussed, since they should be kept in consideration when the extension part will be described.
...
The main window presented to the user is divided into three sections: \emph{Class}, \emph{Current System} and \emph{Current Curve}.
...
\newline
On the other hand the lack of code documentation, such as comments or method contracts, especially in some regions of the code can make reading, understanding and subsequentially modifying the software quite a difficult task.

\chapter{Extending the graphical user interface of MatCont}
...
As written in  \hyperref[sec:additionalFields]{section 7.1.2} the binary file (.mat) associated to a system contains now some additional fields, this could make someone question whether the extended version of MatCont will still work with systems (and thus files) created with the regular version of MatCont. \newline
...
\end{itemize}
Each intermediate version was followed by a small testing phase, consisting primary of Unit testing and a code documentation phase. Towards the ending phases of the development some system testing (e.g. higher level testing aiming to check the quality of the whole system \cite{softwareTesting}) took place. These last tests were carried out with ad-hoc dynamical systems and by creating, studying and comparing the results of some of the examples studied with DDE-BifTool\footnote{A software for the bifurcation analysis of delay-differential equations} that can be found on its reference webpage. page\footnote{http://ddebiftool.sourceforge.net/demos/index.html}.  
