...
\begin{equation}  
\label{logisticEQsol}
\phi^t(y_0)=\frac{K}{1+(\frac{K}{y_0}-1)e^{-r(t)}} cambia t>=0
\end{equation}

The system presents two equilibria: $e_0=0$ and $e_1=K$. For what concerns positive values of $r$, looking at Figure \ref{fig:logistic} it can be seen that all the solutions approach to $e_1$ ($e_1$ it is asymptotically stable) while they move avay from the trivial equibirum ($e_0$ is asymptotically unstable). 
\end{exmp}
...
Let $y_t(\theta):=y(t+\theta)$ with $\theta \in [-\tau,0]$ be the history of the unknown function, which also represents the state, a delay equation can then be defined as 
\begin{equation} 
y'(t)=F(y_t), \;\;\; t \geq 0
\end{equation} 
where the state space is $X=C([-\tau, 0]; {\rm I\!R}^d)$. Note that state $y_t$ is an infinite-dimensional object (a function) \cite{diekmann2012delay}.
\newline 
\newline 
...
\end{cases}\,.
\end{equation}
For what concerns the IVP, the theorem of existence and uniqueness of the solution is still valid. %inserisci riferimento a dieckmann
